...
class _CreatePOScreenState extends State<CreatePOScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (auth.user?.storeId != null && auth.user!.storeId! > 0) {
        _loadAndLockStore(auth.user!.storeId!);
      }
    });
  }

  Future<void> _loadAndLockStore(int storeId) async {
    try {
      final storesList = await _storeService.getAllStores();
      final matched = storesList.where((s) => s.id == storeId).firstOrNull;
      if (matched != null && mounted) {
        setState(() {
          _selectedStore = matched;
        });
      }
    } catch (e) {
      debugPrint("Error loading pre-assigned store in PO screen: $e");
    }
  }
  ...
  Widget _buildSelectionSection() {
    final auth = context.read<AuthProvider>();
    final hasLockedStore = auth.user?.storeId != null && auth.user!.storeId! > 0;
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(12),
          border: Border.all(color: error ? Colors.red.withOpacity(0.3) : (isLocked ? Colors.green.shade200 : Colors.grey[200]!)),
        ...
        child: Row(
          children: [
            Container(
              ...
              decoration: BoxDecoration(color: (error ? Colors.red : (isLocked ? Colors.green : Colors.blue)).withOpacity(0.1), borderRadius: BorderRadius.circular(10)),
              child: Icon(icon, color: error ? Colors.red : (isLocked ? Colors.green : Colors.blue)),
            ),
            ...
            Icon(isLocked ? Icons.lock_outline : Icons.keyboard_arrow_right, color: isLocked ? Colors.green : Colors.grey),
          ],
        ),
      ),
    );
